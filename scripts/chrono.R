cvs_file = paste("output/chrono_", row, "x", column, ".csv", sep="");
title = paste("Chronogramme du pixel [", row, ", ", column, "]", sep="");

A <- matrix(scan(cvs_file, n = 3 * count, sep = ','), count, 3);
A <- t(A);

plot(A[1,], type="l", col="blue", ylim=c(0, 255), xlab="Image no.", ylab="Intensite", main=title)
lines(A[2,], type="l", col="green")
lines(A[3,], type="l", col="red")

x = 1:count

mean_blue = array(mean(A[1,]), c(1, count))
mean_green = array(mean(A[2,]), c(1, count))
mean_red = array(mean(A[3,]), c(1, count))

lines(x, mean_blue, col="blue", lty=3)
lines(x, mean_green, col="green", lty=3)
lines(x, mean_red, col="red", lty=3)

label_blue = paste("Mean = ", format(mean(A[1,]), digits=4), sep="")
label_green = paste("Mean = ", format(mean(A[2,]), digits=4), sep="")
label_red = paste("Mean = ", format(mean(A[3,]), digits=4), sep="")

legend(x=1, y=255, legend=c(label_blue, label_green, label_red), lty=c(3, 3, 3), col=c("blue", "green", "red"))
